...
onMounted(async ()=>{
  ...
  await movieList.getSingleMovie(props.id)
...
<template>
  <section class="bg-white dark:bg-gray-900 m-6 p-4">
    <div class="container" v-if="movieList.isLoading">Is Loading...</div>
    <div v-else class="container flex flex-col items-center px-4 py-12 mx-auto xl:flex-row">
      <div class="flex justify-center xl:w-1/2">
        <img
            ...
            :src="movieList.singleMovie.poster"
            :alt="movieList.singleMovie.title"
        />
      </div>
      ...
      <div class="flex flex-col items-center mt-6 xl:items-start xl:w-1/2 xl:mt-0">
        <h2 class="text-3xl font-bold tracking-tight text-gray-800 xl:text-4xl dark:text-white">
          {{ movieList.singleMovie.title }}
        </h2>
        <span class="m-2 p-2 bg-slate-300 text-slate-800 rounded-md">
            {{  movieList.singleMovie.year }}
          </span>
        <span class="m-2 p-2 bg-slate-300 text-slate-800 rounded-md">
            {{ movieList.singleMovie.runtime }}
          </span>
